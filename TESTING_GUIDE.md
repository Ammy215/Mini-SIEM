...
| W14 🖱️ | A rejected sign-in explains itself | on `/login`, sign in as `a@b` with any password — an address the browser's own `type="email"` accepts but the API rejects | the form stays on screen with "value is not a valid email address…". It must never blank: FastAPI returns `detail` as a **list of objects** for validation errors, and rendering that list directly threw "Objects are not valid as a React child" and left the page empty |
| W15 🖱️ | Every page, three widths, a11y | load each page at 1500, 1024 and 390px as admin, analyst and viewer, with axe | zero serious or critical violations anywhere; no sideways page scroll; no text overflowing its box. Specifically: every filter, switch and input has a name, expandable rows open from the keyboard, and any region that scrolls can be scrolled without a mouse |
| W16 🖱️ | Charts say what they mean | open the dashboard with a few thousand events in range, at 1500 and 1024px | the Activity chart's Y axis shows its real scale (0, 550, 1.1k, 1.6k, 2.2k) rather than a column of clipped zeros, and each donut keeps its legend — slice names and counts both visible, nothing past the card edge |
