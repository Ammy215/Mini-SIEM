// ISO 3166-1 numeric -> alpha-2 for every country shape in world-atlas's
// countries-110m.json. The API reports countries by alpha-2 (ip_geo.country);
// the map's shapes are keyed by numeric code.
...
// Shapes with no ISO numeric code. Northern Cyprus and Somaliland have no
// alpha-2 code either, so IP locations never name them.
const NAME_TO_ALPHA2 = { Kosovo: "XK" };

export function alpha2ForFeature(feature) {
  return NUMERIC_TO_ALPHA2[feature.id] ?? NAME_TO_ALPHA2[feature.properties?.name] ?? null;
}
